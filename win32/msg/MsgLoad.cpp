...
const int RESOURCE_IDS[NUM_MESSAGE_MAX] = {
	...
	IDS_TVP_INSUFFICIENT_NUMBER_OF_ARRAYS,
	IDS_TVP_CAPTURE_AREA_OVER,
	IDS_TVP_ARRAY_IS_ZERO,
	IDS_TVP_FAILED_TO_OPEN_STORAGE_READONLY_MEDIA,
	IDS_TVP_NOT_HAVE_PRIMARY_LAYER,
	IDS_TVP_SET_VIDEO_BUFFER_PARAMETER_ERROR,
	IDS_TVP_CANNOT_ALLOCATE_SDLTEXTURE,
	IDS_TVP_CANNOT_RETRIVE_LAYER_TREE_OWNER_INTERFACE,
	IDS_TVP_CANNOT_CREATE_LOW_LEVEL_GRAPHICS_SYSTEM_MAYBE_LOW_MEMORY,
	IDS_TVP_CANNOT_EXCHANGE_TEXTURE,
	IDS_TVP_CANVAS_BEGIN_EFFECT_NO_CANVAS_SIZE_CALL_WHILE_DRAWING,
	IDS_TVP_CANVAS_BEGIN_STENCIL_CLIP_NO_CANVAS_SIZE_CALL_WHILE_DRAWING,
	IDS_TVP_FBOCREATE_ERROR,
	IDS_TVP_FAILD_TO_CREATE_AUDIO_STREAM,
	IDS_TVP_FAILED_TO_CREATE_THREAD,
	IDS_TVP_FAILED_TO_OPEN_USER_STORAGE,
	IDS_TVP_FILENAME_EXTENSION_NOT_FOUND,
	IDS_TVP_GLCOMPOSITOR1ST_ARGUMENT_MUST_BE_AWINDOW,
	IDS_TVP_GLCOMPOSITOR_FAILED_TO_GET_GLCONTEXT,
	IDS_TVP_INCOMPATIBLE_TEXTURE,
	IDS_TVP_INDEX_TYPE_IS_INVALID,
	IDS_TVP_INVALID_FORMAT_BITS_PER_SAMPLE,
	IDS_TVP_INVALID_FORMAT,
	IDS_TVP_MODAL_WINDOW_IS_NOT_SUPPORTED,
	IDS_TVP_REPLMODAL_TIMEOUT_NO_MODALRESP_WAS_WRITTEN_WITHIN_THE_TIMEOUT_REPL,
	IDS_TVP_SHADER_COMPILE_ERROR,
	IDS_TVP_THIS_METHOD_REQUIRE_VBO,
	IDS_TVP_THIS_VERTEX_BUFFER_IS_CONSTANT,
	IDS_TVP_UNABLE_TO_INITIALIZE_OPEN_GL,
	IDS_TVP_UNABLE_TO_INITIALIZE_GLAD_EGL,
	IDS_TVP_UNABLE_TO_LOAD_GLAD_GLESN,
	IDS_TVP_UNKNOWN_DATA_TYPE,
	IDS_TVP_VERTEX_BINDER_IS_EMPTY,
	IDS_TVP_CANNOT_GET_LAYER_EVENT_TARGET_INTERFACE,
	IDS_TVP_CANNOT_SERIALIZE_CYCLIC_REFERENCE_TO_JSON,
	IDS_TVP_CANNOT_SERIALIZE_NON_FINITE_REAL_INF_NAN_TO_JSON,
	IDS_TVP_CANNOT_SERIALIZE_OBJECT_TO_JSONONLY_DICTIONARY_ARRAY_SUPPORTED,
	IDS_TVP_CANNOT_SERIALIZE_OCTET_TO_JSON,
	IDS_TVP_SET_VIEWPORT_WALLPAPER_WALLPAPER_MUST_BE_ASTRING_LAYER_OR_BITMAP,
	IDS_TVP_NOT_FOUND_IN_SHADER,
	IDS_TVP_CANNOT_RETRIVE_INSTANCE,
	IDS_TVP_PARAMETER_REQUIRE_CLASS_INSTANCE,
	IDS_TVP_UNIMPLEMENTED_CHARACTER_DATA_FOR_FULL_COLORED,
	IDS_TVP_LICENSE_LIST_HEADER,
	IDS_TVP_LICENSE_LIST_FOOTER,
	IDS_TVP_LICENSE_NOT_FOUND,
};
void TVPLoadMessage() {
	...
	TVPInsufficientNumberOfArrays.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_INSUFFICIENT_NUMBER_OF_ARRAYS] );
	TVPCaptureAreaOver.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_CAPTURE_AREA_OVER] );
	TVPArrayIsZero.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_ARRAY_IS_ZERO] );
	TVPFailedToOpenStorageReadonlyMedia.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_FAILED_TO_OPEN_STORAGE_READONLY_MEDIA] );
	TVPNotHavePrimaryLayer.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_NOT_HAVE_PRIMARY_LAYER] );
	TVPSetVideoBufferParameterError.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_SET_VIDEO_BUFFER_PARAMETER_ERROR] );
	TVPCannotAllocateSDLTexture.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_CANNOT_ALLOCATE_SDLTEXTURE] );
	TVPCannotRetriveLayerTreeOwnerInterface.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_CANNOT_RETRIVE_LAYER_TREE_OWNER_INTERFACE] );
	TVPCannotCreateLowLevelGraphicsSystemMaybeLowMemory.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_CANNOT_CREATE_LOW_LEVEL_GRAPHICS_SYSTEM_MAYBE_LOW_MEMORY] );
	TVPCannotExchangeTexture.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_CANNOT_EXCHANGE_TEXTURE] );
	TVPCanvasBeginEffectNoCanvasSizeCallWhileDrawing.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_CANVAS_BEGIN_EFFECT_NO_CANVAS_SIZE_CALL_WHILE_DRAWING] );
	TVPCanvasBeginStencilClipNoCanvasSizeCallWhileDrawing.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_CANVAS_BEGIN_STENCIL_CLIP_NO_CANVAS_SIZE_CALL_WHILE_DRAWING] );
	TVPFBOCreateError.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_FBOCREATE_ERROR] );
	TVPFaildToCreateAudioStream.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_FAILD_TO_CREATE_AUDIO_STREAM] );
	TVPFailedToCreateThread.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_FAILED_TO_CREATE_THREAD] );
	TVPFailedToOpenUserStorage.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_FAILED_TO_OPEN_USER_STORAGE] );
	TVPFilenameExtensionNotFound.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_FILENAME_EXTENSION_NOT_FOUND] );
	TVPGLCompositor1stArgumentMustBeAWindow.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_GLCOMPOSITOR1ST_ARGUMENT_MUST_BE_AWINDOW] );
	TVPGLCompositorFailedToGetGLContext.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_GLCOMPOSITOR_FAILED_TO_GET_GLCONTEXT] );
	TVPIncompatibleTexture.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_INCOMPATIBLE_TEXTURE] );
	TVPIndexTypeIsInvalid.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_INDEX_TYPE_IS_INVALID] );
	TVPInvalidFormatBitsPerSample.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_INVALID_FORMAT_BITS_PER_SAMPLE] );
	TVPInvalidFormat.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_INVALID_FORMAT] );
	TVPModalWindowIsNotSupported.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_MODAL_WINDOW_IS_NOT_SUPPORTED] );
	TVPREPLModalTimeoutNoModalrespWasWrittenWithinTheTimeoutRepl.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_REPLMODAL_TIMEOUT_NO_MODALRESP_WAS_WRITTEN_WITHIN_THE_TIMEOUT_REPL] );
	TVPShaderCompileError.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_SHADER_COMPILE_ERROR] );
	TVPThisMethodRequireVBO.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_THIS_METHOD_REQUIRE_VBO] );
	TVPThisVertexBufferIsConstant.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_THIS_VERTEX_BUFFER_IS_CONSTANT] );
	TVPUnableToInitializeOpenGL.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_UNABLE_TO_INITIALIZE_OPEN_GL] );
	TVPUnableToInitializeGladEGL.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_UNABLE_TO_INITIALIZE_GLAD_EGL] );
	TVPUnableToLoadGladGLESN.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_UNABLE_TO_LOAD_GLAD_GLESN] );
	TVPUnknownDataType.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_UNKNOWN_DATA_TYPE] );
	TVPVertexBinderIsEmpty.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_VERTEX_BINDER_IS_EMPTY] );
	TVPCannotGetLayerEventTargetInterface.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_CANNOT_GET_LAYER_EVENT_TARGET_INTERFACE] );
	TVPCannotSerializeCyclicReferenceToJSON.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_CANNOT_SERIALIZE_CYCLIC_REFERENCE_TO_JSON] );
	TVPCannotSerializeNonFiniteRealInfNanToJSON.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_CANNOT_SERIALIZE_NON_FINITE_REAL_INF_NAN_TO_JSON] );
	TVPCannotSerializeObjectToJSONOnlyDictionaryArraySupported.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_CANNOT_SERIALIZE_OBJECT_TO_JSONONLY_DICTIONARY_ARRAY_SUPPORTED] );
	TVPCannotSerializeOctetToJSON.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_CANNOT_SERIALIZE_OCTET_TO_JSON] );
	TVPSetViewportWallpaperWallpaperMustBeAStringLayerOrBitmap.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_SET_VIEWPORT_WALLPAPER_WALLPAPER_MUST_BE_ASTRING_LAYER_OR_BITMAP] );
	TVPNotFoundInShader.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_NOT_FOUND_IN_SHADER] );
	TVPCannotRetriveInstance.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_CANNOT_RETRIVE_INSTANCE] );
	TVPParameterRequireClassInstance.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_PARAMETER_REQUIRE_CLASS_INSTANCE] );
	TVPUnimplementedCharacterDataForFullColored.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_UNIMPLEMENTED_CHARACTER_DATA_FOR_FULL_COLORED] );
	TVPLicenseListHeader.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_LICENSE_LIST_HEADER] );
	TVPLicenseListFooter.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_LICENSE_LIST_FOOTER] );
	TVPLicenseNotFound.AssignMessage( RESOURCE_MESSAGE[NUM_TVP_LICENSE_NOT_FOUND] );
}

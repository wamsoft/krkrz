...
//---------------------------------------------------------------------------
// 毎フレーム/周期イベントの連続例外ガード
//
// onDraw / Timer.onTimer のような繰り返し発火するイベントのハンドラが
// 例外を投げ続けると、(特に REPL 駆動時のように例外で終了しない構成で)
// 毎フレーム同じ例外が延々と発生し続ける。発火元ごとにこのガードを持ち、
// 連続例外が上限に達したら発火元を止める (Timer なら enabled=false、
// onDraw なら発火停止) ための共通カウンタ。
//
// 上限はコマンドラインオプション -eventexceptionlimit=N (既定 10、
// 0 で無効 = 従来どおり止めない)。
//---------------------------------------------------------------------------
extern tjs_int TVPGetEventExceptionLimit();
struct tTVPRepeatedExceptionGuard
{
	tjs_int Consecutive = 0;

	// 正常終了 1 回 (連続カウンタをリセット)
	void OnSuccess() { Consecutive = 0; }
	// 明示リセット (再開操作時に呼ぶ)
	void Reset() { Consecutive = 0; }
	// 例外 1 回を記録。上限到達なら true (=呼び出し側は発火元を止める) を
	// 返し、カウンタをリセットする。上限 0 (無効) なら常に false。
	bool OnException();
};
//---------------------------------------------------------------------------

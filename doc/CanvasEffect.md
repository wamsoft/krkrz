...
gles プラグイン (`GLESAdaptor`) で拡張されていたポストエフェクト機構と
クリッピング機構を、本体の `Canvas` クラスに取り込んだもの。コマンド仕様は
プラグインと互換で、プラグイン前提で書かれた加工処理を本体の Canvas 描画
(OGLDrawDevice) 単体で再現できる。

実装: `common/visual/opengl/GLEffect.{h,cpp}` (エフェクトチェーン) /
`GLClip.{h,cpp}` (マスク・ステンシル) / `CanvasIntf.cpp` (Canvas 統合)。
...
から、現在の `blendMode` で直前の描画先へ合成する。ネスト可。
`commands` を省略すると素通し合成。加工は CPU 読み戻しなしに GPU 内で完結する。
...
プラグイン、および吉里吉里 GPU 描画の blendmode GLSL に合わせている。
ただし以下は原理的に完全一致せず、視覚的同等を狙う:
...
そのまま使う。scissor はエフェクト / マスクの**合成時にも適用**される。
...
- `beginStencilClip` はマスク α をステンシルへ書き込んで切り抜く。自前で
  ステンシルを使う描画とは競合するので、その場合は mask 方式を使う。
  Offscreen (renderTarget) の FBO は D24S8 付きなのでオフスクリーンでも機能する。

## 注意事項

- シェーダは GLSL ES 3.00。GLES2 フォールバック環境 (EGL 2.0 コンテキスト)
  ではエフェクトはコンパイルに失敗して素通しになる (ログに出る)。
  マスク / ステンシルクリップは ES2 シェーダなので動作する。
- `beginEffect` / `beginMaskClip` の閉じ忘れは `EndDrawing` (フレーム終端) で
  破棄され、警告ログが出る。
- Canvas の既定シェーダの `a_opacity` uniform は**既定値 1.0** (Canvas 生成時に
  設定される)。半透明にしたい場合のみ `canvas.defaultShader.a_opacity` を変更する。
  自作シェーダで `a_opacity` を宣言した場合は従来どおり自分で設定が必要
  (未設定の uniform は GL 初期値 0 = 完全透明)。
